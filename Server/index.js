...
app.post("/api/message", (req, res) => {
  ...
  const sqlInsert =
    "INSERT INTO message (email, subject, description) VALUES (?,?,?);";
  // if (err) {
  //   console.log(err);
  // }
  db.query(sqlInsert, [email, subject, description], (err, result) => {
    console.log(err);
  });
});
...
app.get("/data", (req, res) => {
  ...
  const sqlSelect = "SELECT * FROM fyp_dataset_train  ;";
  db.query(sqlSelect, (err, result) => {
    if (err) {
      console.log(err);
    } else {
